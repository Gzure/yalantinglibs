...
class urma_socket_t {
 public:
  ...
  struct urma_socket_info {
    ...
    // / urma_ubva_t which have packed attributes and unions that are not
 ...
 private:
  void init(config_t config) {
    ...
    for (int i = 0; i <= URMA_MAX_PRIORITY; ++i) {
      ELOG_INFO << "URMA priority_info[" << i << "]: tp_type="
                << cap.priority_info[i].tp_type.value
                << ", SL=" << cap.priority_info[i].SL;
    }
  ...
  urma_socket_info make_local_info() const {
    ...
    // Flatten buffer pool segment info for handshake exchange.
